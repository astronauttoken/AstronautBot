...
def generateCard(user):
    ...
    with Image.open('AstroBingoCard.png') as img:
        ...
        font = ImageFont.truetype(FONT,  FONTSIZE_NUMBER)
        ...
        draw.text((500,920), '@{0}'.format(user), (255,255,0) ,font=ImageFont.truetype(FONT,  FONTSIZE_USERNAME) )
        #ROW 1
...
def main():
    ...
    updater.start_webhook(listen="0.0.0.0",
                          port=int(PORT),
                          url_path=TOKEN)
    updater.bot.setWebhook(HEROKU_APPNAME + TOKEN)
    updater.idle()
